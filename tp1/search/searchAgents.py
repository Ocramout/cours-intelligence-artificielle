        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if (not hitsWall):
                ...
                if (position in self.corners and position not in visited):
                    successors.append(((position, visited + [position]), action, 1))
                else:
                    successors.append(((position, visited), action, 1))
...
def cornersHeuristic(state, problem):
    ...
    cost = 0
    while (unvisited != []):
        distance, corner = min([(util.manhattanDistance(position, c), c) for c in unvisited])
        cost += distance
        position = corner
